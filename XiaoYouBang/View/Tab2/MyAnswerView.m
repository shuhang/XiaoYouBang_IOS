...
@interface MyAnswerView()<MyAnswerTableViewCellDelegate>
{
    UITableView * tableView;
}
@end

@implementation MyAnswerView
...
- ( void ) headerRereshing
{
    ...
    [[NetWork shareInstance] httpRequestWithPostPut:@"api/answers/me" params:request method:@"POST" success:^(NSDictionary * result)
     {
...
- (void)tableView:(UITableView *)tableView_ didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if( [self.delegate respondsToSelector:@selector(clickAnswer_mine:)] )
    {
        [self.delegate clickAnswer_mine:[self.answerArray objectAtIndex:indexPath.row]];
    }
}
    ...
    [[NetWork shareInstance] httpRequestWithGet:url params:request success:^(NSDictionary * result)
     {
         if( [result[ @"result" ] intValue] == 3000 )
         {
             ...
             if( [self.delegate respondsToSelector:@selector(loadQuestionSuccess_mine:)] )
             {
                 [self.delegate loadQuestionSuccess_mine:entity];
             }
         }
         else
         {
